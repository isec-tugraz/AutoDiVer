...
void key_schedule(const uint8_t *key, uint32_t* round_keys, int num_rounds_ks)
{
    int r;
...
void pyjamask_96_enc(const uint8_t *plaintext, const uint8_t *key, uint8_t *ciphertext, int num_rounds_ks, int num_rounds)
{
    ...
    uint32_t round_keys[4*(num_rounds_ks+1)];

    key_schedule(key, round_keys, num_rounds_ks);
    ...
    add_round_key_96(state, round_keys, num_rounds);
...
void pyjamask_96_enc_longkey(const uint8_t *plaintext, const uint32_t *long_key, uint8_t *ciphertext, int num_rounds)
{
    ...
    for (r=0; r<num_rounds; r++)
    {
        add_round_key_96(state, long_key, r);
        sub_bytes_96(state);
        mix_rows_96(state);
    }

    add_round_key_96(state, long_key, num_rounds);
...
void pyjamask_96_dec(const uint8_t *ciphertext, const uint8_t *key, uint8_t *plaintext, int num_rounds_ks, int num_rounds)
{
    int r;
    uint32_t state[STATE_SIZE_96];
    uint32_t round_keys[4*(num_rounds_ks+1)];
...
void pyjamask_128_enc(const uint8_t *plaintext, const uint8_t *key, uint8_t *ciphertext, int num_rounds_ks, int num_rounds)
{
    int r;
    ...
    add_round_key_128(state, round_keys, num_rounds);
...
void pyjamask_128_dec(const uint8_t *ciphertext, const uint8_t *key, uint8_t *plaintext, int num_rounds_ks, int num_rounds)
{
    int r;
    uint32_t state[STATE_SIZE_128];
    uint32_t round_keys[4*(num_rounds_ks+1)];
