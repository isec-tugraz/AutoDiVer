...
#distutils: sources = src/differential_verification/gift128/gift128_ref.c
cimport cython
...
def gift128_enc(const uint8_t[::1] pt not None, const uint8_t[::1] key not None, unsigned int numrounds=35) -> uint8_t[:]:
    cdef ssize_t i
    if pt.shape[0] != 32 or key.shape[0] != 32 or numrounds > 28:
