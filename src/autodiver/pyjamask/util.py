...
def load_state(plaintext: np.ndarray[Any, np.dtype[np.int8]], state_size) -> np.ndarray[Any, np.dtype[np.uint32]]:
...
def unload_state(state: np.ndarray[Any, np.dtype[np.uint32]], state_size) -> np.ndarray[Any, np.dtype[np.uint8]]:
...
def pyjamask_mat_mult(mat_col: np.uint32, vec: np.uint32) -> np.uint32:
    ...
    for i in range(31, -1, -1):
        mask = ONES if ((vec >> i) & 1) else ZERO
        res = res ^ (mask & mat_col)
        mat_col = (mat_col >> np.uint32(1)) | (mat_col << np.uint32(31))

    return res

def pyjamask_mix_rows_96(state: np.ndarray[Any, np.dtype[np.uint32]]) -> np.ndarray[Any, np.dtype[np.uint32]]:
